...
const FarmModal: React.FC<FarmModalProps> = ({ farm, onClose, onSave }) => {
	...
	const validateForm = (): boolean => {
		...
		if (!latitude.trim()) newErrors['latitude'] = "La latitud es obligatoria.";
    if (Number(latitude) < -90 || Number(latitude) > 90) newErrors['latitude'] = "La latitud debe estar entre -90 y 90.";
		if (!longitude.trim()) newErrors['longitude'] = "La longitud es obligatoria.";
		if (Number(longitude) < -180 || Number(longitude) > 180) newErrors['longitude'] = "La longitud debe estar entre -180 y 180.";
	...
	return (
		<div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
		  <div
			className="bg-white p-8 rounded-lg shadow-lg w-full max-w-4xl max-h-screen overflow-y-auto"
			style={{ maxHeight: "90vh" }} // Limitar la altura máxima del modal
		  >
