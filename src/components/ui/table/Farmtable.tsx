...
const FarmTable: React.FC<FarmTableProps> = ({ farms, onEdit, onDelete, total, page, limit, setPage, setLimit }) => {
  ...
  const [farmIdToDelete, setFarmIdToDelete] = useState<number | null>(null);
  ...
  const handleEdit = (farm: Farm):void => {
    const farmRequest: FarmRequest = {
      ...
      users: farm.users.map(user => (user as User).id)
    };
    onEdit(farmRequest);
  };

  const handleDelete = (farmId: number):void => {
    setFarmIdToDelete(farmId);
    setIsConfirmModalOpen(true);
  };

  const confirmDelete = ():void => {
    if (farmIdToDelete !== null) {
      onDelete(farmIdToDelete);
  ...
  return (
    <div>
        ...
        <tbody>
          {farms.map((farm, index) => (
            <tr key={farm.id}>
              ...
              <td className="py-2">
                <ButtonComponent onClick={() => { handleEdit(farm); }}>Editar</ButtonComponent>
                <ButtonComponent className="ml-2" onClick={() => { handleDelete(farm.id); }}>Eliminar</ButtonComponent>
              </td>
            </tr>
          ))}
        </tbody>
      ...
      {isConfirmModalOpen && (
        <ConfirmModal
          message="¿Estás seguro de que deseas eliminar esta granja?"
